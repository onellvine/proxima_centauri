...
class CustomAccountManager(BaseUserManager):
    ...
    def create_user(self, email, user_name, first_name, phone, password, **other_fields):
        ...
        user = self.model(email=email, user_name=user_name, first_name=first_name, phone=phone, **other_fields)
        other_fields.setdefault('is_active', True) # enable all created users to be active
...
class NewUser(AbstractBaseUser, PermissionsMixin):
    ...
    phone = models.CharField(max_length=13, help_text=_("Phone Format: +254712345678"), unique=True)
